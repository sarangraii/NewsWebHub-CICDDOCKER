...
class NotificationService:
    ...
    def setup_firebase(self):
        """Initialize Firebase Admin SDK using Render Secret File"""
        try:
            # Read path from environment variable
            firebase_path = os.getenv("FIREBASE_CREDENTIALS_PATH")
            print("🔥 FIREBASE_CREDENTIALS_PATH =", firebase_path)

            if not firebase_path:
                print("⚠️ FIREBASE_CREDENTIALS_PATH not set - notifications disabled")
                return
            ...
            if not cred_path.exists():
                ...
                if Path("/etc/secrets").exists():
                    print("📂 /etc/secrets contents:", os.listdir("/etc/secrets"))
                return
    ...
    async def send_topic_notification(self, article: dict, topic: str):
        """Send notification for a specific topic"""
        if not self.initialized:
            return
