...
def serialize_news(doc) -> dict:
    return {
        ...
        "content": doc.get("content", ""),
        "url": doc.get("url", ""),
        "urlToImage": doc.get("urlToImage"),
        "publishedAt": doc.get("publishedAt"),
        "source": doc.get("source", {"name": "Unknown"}),
        "language": doc.get("language", "en"),
        "category": doc.get("category", "general"),
        "aiSummary": doc.get("aiSummary"),
        "audioSummaryUrl": doc.get("audioSummaryUrl"),
...
async def get_news(
    ...
    articles = [serialize_news(doc) async for doc in cursor]
...
async def get_latest_news(limit: int = Query(10, ge=1, le=50)):
    ...
    return [serialize_news(doc) async for doc in cursor]
...
async def summarize_article(article_id: str):
    ...
    audio_url = ai_summarizer.generate_audio_summary(
        summary, doc.get("language", "en")
    )
